...
typeCheckUnOp NotOp t = ensure t boolType >> return boolType
typeCheckUnOp NegOp t = ensure t numType >> return numType
...
typeCheckBinOp op = case op of
 ...
 where
  boolOp left right = ensure left boolType >> ensure right boolType >> return boolType
  numOp left right = ensure left numType >> ensure right numType >> return numType
...
typeCheckStatement ns retType (CodeBlock stmts) =
  foldM (`typeCheckStatement` retType) ns stmts
